...
class SearchView(ListView):
    ...
    def get_queryset(self):
        ...
        query = Q()
        if position:
            query |= (Q(title__icontains=position) | 
                     Q(description__icontains=position) |
                     Q(company_name__icontains=position))
        if location:
            # Split location into parts and search for each part
        ...
        if position or location:
            return self.model.objects.filter(query).distinct().order_by('-created_at')
    ...
    def get_context_data(self, **kwargs):
        ...
        if 'position' in self.request.GET:
            context['position'] = self.request.GET['position']
